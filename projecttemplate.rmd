...
ggplot(aes(x = APM, color = factor(LeagueIndex)), data = sc) +
  geom_histogram(binwidth = 50) +
...
ggplot(aes((MinimapAttacks + MinimapRightClicks), APM, color = APM_bucket), 
       data = sc) + 
...
ggplot(aes(factor(LeagueIndex), (MinimapAttacks + MinimapRightClicks)), 
           data = sc) +
  geom_boxplot(aes(color = APM_bucket)) + 
...
ggplot(aes((MinimapAttacks + MinimapRightClicks), APM, color = APM_bucket), 
  ...
  scale_color_discrete(name = "APM Range")
```

### Description Three

As we move up in league index, we can see a shift towards the right of each plot indicating that better players utilize minimap attacks and minimap right clicks more. In addition, it follows the trend that the best players are utilizing more available game mechanics than worse players. Both grandmaster and pro level players are using far more minimap clicks whether it be for scouting or moving army while doing other things. 
...
The other big limitation I find with this data is that it's a snapshot at the end of the game. I would have liked to examine APM ranges within the game, peak actions, and when certain actions such as map exploration happens, and how everything contributes to whether a player won or lost. 
