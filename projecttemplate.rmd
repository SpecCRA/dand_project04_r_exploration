...
At first glance, it looks like APM, PAC data, and hot key usage the strongest predictor of league placement as expected. A little surprisingly, age has little correlation to any of the variables. I'm most interested in the relationship between a player's action variables (APM, PAC data, workers made, etc) and its relation to league placement to examime what separates each league and more generally, what are elite players doing more of?

```{r echo=FALSE, Bivariate_plots_cont}
ggplot(aes(factor(LeagueIndex), Age), data = sc) + 
  geom_boxplot()
```
