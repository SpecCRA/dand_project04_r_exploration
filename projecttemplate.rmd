...
```{r echo=FALSE, message=FALSE, Time_data}
...
```{r echo=FALSE, message=FALSE, Univariate_Plots_APM}
...
```{r echo=FALSE, message=FALSE, Univariate_Plots_general}
...
```{r echo=FALSE, message=FALSE, Workers_created_raw}
sc$WorkersCreatedTotal <- sc$WorkersMade * sc$MaxTimeStamp
sc$WorkersCreatedTotal <- round(sc$WorkersCreated)
...
# Okay, this first one didn't actually make sense. I just used it to test output first. Game ID is pretty useless.
ggpairs(sc_subset01,
...
Since APM and action latency are the most correlated to league index, I decided to use APM as my basis first. Both are related to activity of a player in general, and it makese sense that the player who does more is better. 

```{r echo = FALSE, APM_plots}
...
After a little more exploration, I'd also like to see what players are spending their available APM doing. 
